...
export type UpdateFiltersState = {
  search: string;
  sort: UpdateSort;
...
type UpdateFiltersProps = {
  filters: UpdateFiltersState;
  onChange: (filters: UpdateFiltersState) => void;
  onReset: () => void;
  layout?: "sidebar" | "mobile";
};

type OpenPanel = "sort" | "category" | null;

function getSortLabel(sort: UpdateSort) {
  return (
    UPDATE_SORT_OPTIONS.find((option) => option.value === sort)?.label ??
    "Newest First"
  );
}
...
function SortPillList({
  ...
  value: UpdateSort;
  onChange: (value: UpdateSort) => void;
}) {
  return (
    <div className="mt-3 flex flex-col gap-2">
      {UPDATE_SORT_OPTIONS.map((option) => {
...
export default function UpdateFilters({
  ...
  const update = (partial: Partial<UpdateFiltersState>) => {
...
export const DEFAULT_UPDATE_FILTERS: UpdateFiltersState = {
  search: "",
  sort: "recent",
