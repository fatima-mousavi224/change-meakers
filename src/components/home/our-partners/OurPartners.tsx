        ...
        {PARTNERS.map((partner) => (
          <div key={partner.id} className="group flex items-center">
            <Image
              ...
              quality={100}
              unoptimized
              className={`w-auto shrink-0 object-contain object-left grayscale transition-[filter] duration-300 group-hover:grayscale-0 ${partner.imageClassName ?? "h-10 max-w-[200px] sm:h-12 sm:max-w-[260px] lg:h-14"}`}
            />
          </div>
        ))}
      </div>
